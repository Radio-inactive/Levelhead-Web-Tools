...
function generateToolFooter() {
  ...
  toolNames.forEach((name) => {
    if (name[0].toLowerCase() != currentToolName.toLowerCase())
...
function getExtendedRequestBody(method = "GET") {
  var requestBody = {
    method: method,
    headers: {
      "Rumpus-Delegation-Key": JSON.parse(
...
function checkDelegationKey() {
  try {
    if (window.localStorage.getItem("DelegationKey"))
      fetch(
        "https://www.bscotch.net/api/levelhead/aliases?userIds=@me",
        getExtendedRequestBody("GET")
      ).then(function (r) {
...
function getInteractions(level) {
  ...
  var ownLevel =
    level.userId ==
    JSON.parse(window.localStorage.getItem("DelegationKey")).UserCode;
  ...
  if (level.interactions) {
    ...
    possibleInteractions.bookmarked = level.interactions.bookmarked
      ? true
      : false;
    if (!ownLevel) {
      possibleInteractions.liked = level.interactions.liked ? true : false;
      possibleInteractions.favorited = level.interactions.favorited
        ? true
        : false;
      possibleInteractions.played = level.interactions.played ? true : false;
      possibleInteractions.completed = level.interactions.completed
        ? true
        : false;
    }
  }
...
function manageBookmark(levelCode, mode = "PUT") {
  ...
  try {
    fetch(
      `https://www.bscotch.net/api/levelhead/bookmarks/${levelCode}`,
...
function loadCursor() {
  ...
  if (cursor && cursor != cursorOptions[0] && cursorOptions.includes(cursor)) {
    document.body.style.cursor = setCursorTemplate.replaceAll(
      "{{color}}",
      cursor
    );
...
function timeFormat(time) {
  ...
  if (!format) return timeFormatOptions["standard"](time);
  else return timeFormatOptions[format](time);
}
...
var timeFormatOptions = {
  standard: function (time) {
    ...
    if (time >= 86400)
      return (
        ...
        (
          ...
          "." +
          millis.substr(millis.length - 2, millis.length - 1) +
          "s"
        )
          .replace(":", "h ")
          .replace(":", "m ")
    ...
    return (
      new Date(time * 1000).toISOString().substr(17, 2) +
      "." +
      millis.substr(millis.length - 2, millis.length - 1) +
      "s"
    );
...
function dateFormat(date) {
  ...
  if (!format) return dateFormatOptions["standard"](date);
  else return dateFormatOptions[format](date);
}
...
function scoreFormat(score) {
  ...
  if (!format) return scoreFormatOptions["standard"](score);
  else return scoreFormatOptions[format](score);
}
