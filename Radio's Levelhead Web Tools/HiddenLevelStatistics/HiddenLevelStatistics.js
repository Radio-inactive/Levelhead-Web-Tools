...
function recursivelyLoadLikes(lastId) {
  fetch(
    "https://www.bscotch.net/api/levelhead/levels/" +
      getLevelCode(levelCode.value) +
      "/likes?limit=32&includeAliases=true&beforeId=" +
      lastId
  )
    .then((r) => r.json())
    .then(function (r) {
        ...
        likes.forEach((x) => {
          ...
          x.forEach((user) => {
            ...
            if (user.alias.avatarId)
              //ToDo: possibly short if notation?
              htmlout += userTemplate
                .replace("{{userName}}", user.alias.alias)
...
function recursivelyLoadFavorites(lastId) {
  fetch(
    "https://www.bscotch.net/api/levelhead/levels/" +
      getLevelCode(levelCode.value) +
      "/favorites?limit=32&includeAliases=true&beforeId=" +
      lastId
  )
    .then((r) => r.json())
    .then(function (r) {
...
function loadHiddenStatistics() {
  ...
  fetch(
    "https://www.bscotch.net/api/levelhead/levels?levelIds=" +
      getLevelCode(levelCode.value) +
      "&includeAliases=true&includeStats=true&includeRecords=true"
  )
    .then((r) => r.json())
    .then(function (r) {
      if (r.data.length == 1) {
        ...
        document.getElementById("EB").innerHTML = r.data[0].stats.ExposureBucks
          ...
          : 0;
        document.getElementById("ClearRate").innerHTML = r.data[0].stats
          .ClearRate
          ? r.data[0].stats.ClearRate
          : 0;
        document.getElementById("FailureRate").innerHTML = r.data[0].stats
          .FailureRate
          ? r.data[0].stats.FailureRate
          : 0;
        document.getElementById("HiddenGem").innerHTML = r.data[0].stats
          .HiddenGem
          ? r.data[0].stats.HiddenGem
          : 0;
        document.getElementById("ReplayValue").innerHTML = r.data[0].stats
          .ReplayValue
          ? r.data[0].stats.ReplayValue
          : 0;
        document.getElementById("Successes").innerHTML = r.data[0].stats
          .Successes
          ? r.data[0].stats.Successes
          : 0;
        document.getElementById("TimePerWin").innerHTML = r.data[0].stats
          .TimePerWin
          ? timeFormat(r.data[0].stats.TimePerWin)
          : "No Times";
        document.getElementById("UpdatedAt").innerHTML = dateFormat(
          r.data[0].updatedAt
        );
        //record statistics
        var recordHolders = "";
        ...
        for (var i = 0; i < r.data[0].records.FastestTime.length; i++)
          recordHolders += recordHolderTemplate
            .replace(
              "{{userName}}",
              r.data[0].records.FastestTime[i].alias.alias
            )
            .replace(
              "{{userTime}}",
              timeFormat(r.data[0].records.FastestTime[i].value)
            )
            .replace(
              "{{recordDate}}",
              dateFormat(r.data[0].records.FastestTime[i].createdAt)
            );
        ...
        for (var i = 0; i < r.data[0].records.HighScore.length; i++)
          recordHolders += highScoreHolderTemplate
            ...
            .replace(
              "{{userTime}}",
              scoreFormat(r.data[0].records.HighScore[i].value)
            )
            .replace(
              "{{recordDate}}",
              dateFormat(r.data[0].records.HighScore[i].createdAt)
            );
        document.getElementById("HighScore").innerHTML = recordHolders;
      } //ToDo: replace this if else with an if + return at the start
      else document.getElementById("validity").innerHTML = "INVALID CODE!";
    });
}
