...
export module ApplySettings {
     ...
     */
    export function levelFetchUrl(stats = true, aliases = true, records = true, interactions = false, limit = 128) {
     ...
     */
    export function getProfileCode(input = "") {
     ...
     * @returns the cleaned code
     */
    export function getLevelCode(input = "") {
        var clean = input.trim().toLowerCase();
        if (input.length == 7)
            return clean;
     ...
     */
    export function getAnyCode(input = "") {
     ...
     * @returns the cleaned code
     */
    export function getPlaylistCode(input = "") {
        var clean = input.trim().toLowerCase();
        if (input.length == 7)
            return clean;
     ...
     */
    export function getAvatarURL(avatarId: string, size = 100) {
     ...
     */
    export function getExtendedRequestBody(method = 'GET'): RequestInit {
        var requestBody: RequestInit = {
            'method': method,
            'headers': {
     ...
     */
    export function checkDelegationKey() {
        try {
     ...
     */
    export function addInteractionDetails() {
     ...
     */
    export function getInteractions(level: CampaignApi.Level) {
     ...
     */
    export function manageBookmark(levelCode: string, mode = 'PUT') {
        console.log(JSON.parse(window.localStorage.getItem('DelegationKey') || '').Key);
        try {
            fetch(`https://www.bscotch.net/api/levelhead/bookmarks/${levelCode}`, getExtendedRequestBody(mode));
            return true;
        }
        catch (exception) {
     ...
     */
    export function timeFormat(time: number) { //ToDo: document this monstrosity
        var millis = "";
